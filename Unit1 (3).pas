...
procedure encodeABM (path, filename:string);
var
abmBuf, RLEbuf : PByteArray;
abmFileSize, pos1, bmpPos, BMPw, BMPh, totalpics, fsize, idx, RLEidx, j, k, cnt, framesTOTAL, frameNUM, maxRAWsize:longint;
fin, fout:file of Byte;
fname2 : string;
logout, pics2replace : TextFile;
files : array of string;
...
begin
 ...
 maxRAWsize := getDWORD($4, abmBuf);

 GetMem(RLEbuf, abmFileSize*2);
 RLEidx := 0;
...
buf:PByteArray;
begin
 decodeABM('.\files\abm\EDNASIGN.ABM');
 {
 path:='.\files\abm\';
...
begin
encodeABM('.\rus\ANIMATION', 'EDNASIGN');
end;
